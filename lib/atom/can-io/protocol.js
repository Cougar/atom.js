...
Protocol.prototype.encodeEnum = function(values, bitSet, startBit, bitLength, value) {
  var uint_value = parseInt(this.lookupAttr(values, 'id', function(val) {
...
Protocol.prototype.decodeAscii = function(bitSet, startBit, bitLength) {
  ...
  for (var n = 0; n < bitLength; n += 8) {
    value += String.fromCharCode(bitSet.read(startBit + n, 8));
  }
  return value;
};
...
Protocol.prototype.encode = function(message) {
  ...
  header.classId = this.resolveClassId(header.className);

  data[0] = 253; // Start byte
  data[4] = header.classId << 1;

  if (header.className == "nmt") {
    header.commandId = this.resolveNMTCommandId(header.commandName);

    data[3] = header.commandId;
  } else {
    header.directionFlagId = this.resolveDirectionFlagId(header.directionFlagName);
    // default to zero
    header.moduleTypeId = header.moduleTypeName ? this.resolveModuleTypeId(header.moduleTypeName) : 0;
    // default to zero
    header.moduleId = header.moduleId || 0;
    header.commandId = this.resolveCommandId(header.commandName, header.moduleTypeName);

    data[4] |= (header.directionFlagId & 0x01);
    data[3] = header.moduleTypeId;
    data[2] = header.moduleId;
    data[1] = header.commandId;
  }
  ...
  variables.forEach(function(variable) {
    if (typeof message.body[variable.name] === 'undefined') {
      return;
    }
  ...
  header.length = Math.min(Math.ceil(highestBit / 8.0), 8);

  data[5] = 1;
  data[6] = 0;
  data[7] = header.length;
