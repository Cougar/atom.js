...
Protocol.prototype.decodeBody = function(buffer, header) {
  ...
  console.log(header.commandId, header.moduleName);
  var variables = this.lookupCommandVariables(header.commandId, header.moduleName);
  var bitSet = new bitset.BitSet(buffer.slice(8));
  var result = {};
  var self = this;
  variables.forEach(function(variable) {
    var name = variable.name;
    var startBit = variable.start_bit;
    var bitLength = variable.bit_length;
...
Protocol.prototype.decodeUint = function(bitSet, startBit, bitLength) {
  console.log('decoding uint from', startBit, 'of length', bitLength, 'with bits', bitSet.buffer);
  return bitSet.read(startBit, bitLength);
};

Protocol.prototype.decodeFloat = function(bitSet, startBit, bitLength) {
  return this.decodeInt(bitSet.read(startBit, bitLength)) / 64.0;
};
...
Protocol.prototype.lookupNMTCommandName = function(commandId) {
  return this.lookupAttr(this.protocol.root.nmt_messages.nmt_message, 'name', function(msg) {
    return msg.id === commandId.toString();
  });
};
