...
var sessionClass = require('./session');
...
function TransportTCP(port) {
  ...
  this.server.on('listening', function () {
    ...
    console.log('TransportTCP is now listening for clients on ' + address.address + ':' + address.port);
  });
  ...
  this.server.on('error', function(error) {
    if (error.code == 'EADDRINUSE') {
      console.log('TransportTCP port (' + self.port + ') in use, retrying...');
...
TransportTCP.prototype.handleNewConnection = function(identifier) {
  ...
  console.log('Client connected from ' + identifier.remoteAddress +':'+ identifier.remotePort);
